...
def occurrence_candidates(recs, sites, assets):
    ...
    # --- 1. serial cohort x symptom, normalised by installed units ----------
    g = collections.defaultdict(list)
    for r in corrective:
        if r["asset_model"] and r["mfg_quarter"]:
            g[(r["asset_model"], r["mfg_quarter"], r["symptom"])].append(r)
    for (model, q, sym), members in g.items():
        units = pop.get((model, q), 0)
        if len(members) < MIN_CLUSTER or units < 10:
            continue
        rate = len(members) / units
        peer = [len(v) / max(pop.get((m2, q2), 1), 1)
                for (m2, q2, s2), v in g.items() if s2 == sym and (m2, q2) != (model, q)]
        base = statistics.median(peer) if peer else 0.0
        out.append(dict(
            kind="serial_cohort", key=f"{model}|{q}|{sym}",
            label=f"{model} manufactured {q}, symptom '{sym}'",
            ...
            dims=dict(model=model, mfg_quarter=q, symptom=sym)))
...
def run(recs, sites, assets, budget=15):
