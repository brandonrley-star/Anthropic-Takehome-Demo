...
async function renderDetail(cid){
  ...
  const wos = await api('/api/work_order/' + (d.supporting_wo_ids[0]||'')).catch(()=>null);
  ...
  $('#v-findings').innerHTML = `
    ...
    <div class="ask" id="ask">
      ...
      ${st.available ? `
        ...
        <div id="ask-out"></div>`
      : `<div class="warn" style="margin-top:11px">Live Claude Q&amp;A unavailable —
           core analysis is precomputed and remains fully accessible.</div>`}
    </div>
...
$('#strip-next').onclick = () => { demoAt = Math.min(demoAt+1, DEMO.length-1);
  location.hash = DEMO[demoAt][1]; paintStrip(); };
